...
def train(data_by_user_user_index_offsets_train, 
    ...
    model = (user_biases, user_embeddings, movie_biases, movie_embeddings, feature_embeddings)
...
def plot_errors_and_losses(train_losses,  train_errors):    
    ...
    ax.grid(True)
    ax.set_xlabel("Epoch")
    plt.savefig(f"./outputs/plots/feature_and_embedding_model_nll_32m_embed{embedding_dim}_full_dataset.pdf")
    plt.close()
    ...
    ax.set_ylabel("RMSE")
    ax.grid(True)
    plt.savefig(f"./outputs/plots/feature_and_embeddding_model_rmse_32m_embed{embedding_dim}_full_dataset.pdf")
    plt.close()
...
if __name__ == "__main__":
    ...
    np.savez(f"./models/model_with_feature_embeding_dim_{embedding_dim}_32m.npz",
